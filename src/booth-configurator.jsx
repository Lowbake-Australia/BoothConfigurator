...
/* ========== UI ========== */
var A="#f97316",PB="#13161e",SB="#191d28",BC="#262b38",TP="#e4e8f0",TS="#7c8498",ST="#2a2f3e";
...
function LightEditor({width,depth,roofLightConfig,wallLightConfig,maxRoof,numPanels,isDesk,isOF,onUpdateRoof,onUpdateWall,onClose}){
  ...
  return(
    <div style={{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={onClose}>
      <div style={{background:PB,borderRadius:16,padding:24,maxWidth:600,width:"92%",maxHeight:"92vh",overflowY:"auto",border:"1px solid "+BC}} onClick={function(e){e.stopPropagation();}}>
        ...
        {/* SVG Plan */}
        <div style={{background:"#0d0f16",borderRadius:10,padding:12,border:"1px solid "+BC,overflowX:"auto"}}>
          <svg width={svgW} height={svgH} viewBox={"0 0 "+svgW+" "+svgH} style={{display:"block",margin:"0 auto"}}>
            {/* Booth outline */}
            <rect x={boothX} y={boothY} width={width*sc} height={drawD*sc} fill="#1a1e28" stroke="#555" strokeWidth="2" rx="2"/>
            ...
            {panels.map(function(pr){
              ...
              var els=[];
              // Panel divider
              if(pr.idx>0)els.push(<line key={"d"+pr.idx} x1={boothX} y1={pyPx} x2={boothX+width*sc} y2={pyPx} stroke="#444" strokeWidth="1" strokeDasharray="4,4"/>);
          ...
          {panels.map(function(pr){
            ...
            return(
              <div key={pr.idx} style={{padding:"10px 0",borderBottom:pr.idx<panels.length-1?"1px solid "+BC:"none"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:isOF?8:0}}>
                  <div><span style={{fontSize:13,color:TP,fontWeight:600}}>Panel {pr.idx+1}</span><span style={{fontSize:11,color:TS,marginLeft:8}}>{pr.h.toFixed(1)}m</span></div>
                  <div style={{display:"flex",alignItems:"center",gap:4}}>
                    <span style={{fontSize:11,color:TS,marginRight:4}}>Roof:</span>
                    {roofOpts.map(function(n){var active=pr.roof===n;return(<button key={n} onClick={function(){setRoof(pr.idx,n);}} style={{width:32,height:28,fontSize:12,fontWeight:700,border:active?"2px solid "+A:"1px solid "+BC,borderRadius:6,background:active?A:"transparent",color:active?"#fff":TS,cursor:"pointer"}}>{n}</button>);})}
                  </div>
                </div>
                {isOF&&<div style={{display:"flex",alignItems:"center",gap:12,marginTop:4}}>
                  <span style={{fontSize:11,color:TS}}>Walls:</span>
                  {["left","right"].map(function(side){var on=pr.wall&&pr.wall[side];return(
                    <button key={side} onClick={function(){setWall(pr.idx,side,!on);}} style={{padding:"4px 12px",fontSize:11,fontWeight:600,border:on?"2px solid #60a5fa":"1px solid "+BC,borderRadius:6,background:on?"rgba(96,165,250,0.15)":"transparent",color:on?"#60a5fa":TS,cursor:"pointer",textTransform:"capitalize"}}>{side}{on?" ✓":""}</button>);})}
                </div>}
              </div>);
          })}
        </div>
...
export default function BoothConfigurator(){
  ...
  useEffect(function(){var cv=canvasRef.current;if(!cv)return;var rr=new THREE.WebGLRenderer({canvas:cv,antialias:true,alpha:false});rr.setPixelRatio(Math.min(window.devicePixelRatio,2));rr.setClearColor(0x0a0c14);rr.shadowMap.enabled=true;rr.shadowMap.type=THREE.PCFSoftShadowMap;rr.toneMapping=THREE.ACESFilmicToneMapping;rr.toneMappingExposure=1.2;var sc=new THREE.Scene();sc.fog=new THREE.Fog(0x0a0c14,18,40);var cm=new THREE.PerspectiveCamera(45,1,0.1,100);sc.add(new THREE.AmbientLight(0x9eaabb,0.6));var dl=new THREE.DirectionalLight(0xfff5e8,0.9);dl.position.set(6,10,8);dl.castShadow=true;dl.shadow.mapSize.set(2048,2048);dl.shadow.camera.near=0.1;dl.shadow.camera.far=30;dl.shadow.camera.left=-10;dl.shadow.camera.right=10;dl.shadow.camera.top=10;dl.shadow.camera.bottom=-5;sc.add(dl);var fl=new THREE.DirectionalLight(0xc0d0ff,0.3);fl.position.set(-5,4,-3);sc.add(fl);var bl=new THREE.DirectionalLight(0xffeedd,0.15);bl.position.set(0,3,-6);sc.add(bl);var gr=new THREE.Mesh(new THREE.PlaneGeometry(50,50),new THREE.MeshStandardMaterial({color:0x888888,roughness:0.95}));gr.rotation.x=-Math.PI/2;gr.position.y=-0.02;gr.receiveShadow=true;sc.add(gr);var gd=new THREE.GridHelper(30,30,0x555555,0x444444);gd.position.y=-0.01;gd.material.opacity=0.15;gd.material.transparent=true;sc.add(gd);var bg=new THREE.Group();sc.add(bg);sceneRef.current={renderer:rr,scene:sc,camera:cm,boothGroup:bg};var rs=function(){var p=cv.parentElement;if(!p)return;rr.setSize(p.clientWidth,p.clientHeight);cm.aspect=p.clientWidth/p.clientHeight;cm.updateProjectionMatrix();};var ro=new ResizeObserver(rs);ro.observe(cv.parentElement);rs();var id;var lp=function(){id=requestAnimationFrame(lp);rr.render(sc,cm);};lp();return function(){cancelAnimationFrame(id);ro.disconnect();rr.dispose();};},[]);
  useEffect(function(){var cv=canvasRef.current;if(!cv)return;var dn=function(e){dragRef.current={active:true,lastX:e.clientX,lastY:e.clientY,shift:e.shiftKey};};var mv=function(e){if(!dragRef.current.active)return;var dx=e.clientX-dragRef.current.lastX,dy=e.clientY-dragRef.current.lastY;if(dragRef.current.shift||e.shiftKey){var ps=cameraState.current.radius*0.002;cameraState.current.panX-=dx*ps;cameraState.current.panY+=dy*ps;if(cameraState.current.panY<0)cameraState.current.panY=0;}else{cameraState.current.theta-=dx*0.005;cameraState.current.phi-=dy*0.005;}dragRef.current.lastX=e.clientX;dragRef.current.lastY=e.clientY;updateCamera();};var up=function(){dragRef.current.active=false;};var wh=function(e){e.preventDefault();cameraState.current.radius+=e.deltaY*0.01;updateCamera();};cv.addEventListener("pointerdown",dn);window.addEventListener("pointermove",mv);window.addEventListener("pointerup",up);cv.addEventListener("wheel",wh,{passive:false});return function(){cv.removeEventListener("pointerdown",dn);window.removeEventListener("pointermove",mv);window.removeEventListener("pointerup",up);cv.removeEventListener("wheel",wh);};},[updateCamera]);
  ...
  return(
    <div style={{display:"flex",height:"100vh",width:"100vw",background:"#0a0c14",fontFamily:"'SF Pro Display','Segoe UI',system-ui,sans-serif",overflow:"hidden"}}>
      <div style={{flex:1,position:"relative",minWidth:0}}>
        <canvas ref={canvasRef} style={{display:"block",width:"100%",height:"100%",cursor:"grab"}}/>
        <div style={{position:"absolute",top:20,left:24,pointerEvents:"none"}}><div style={{fontSize:11,fontWeight:700,letterSpacing:"0.2em",color:A,textTransform:"uppercase",opacity:0.9}}>At The Booth</div><div style={{fontSize:18,fontWeight:700,color:TP,marginTop:2}}>{boothLabel}</div></div>
        <div style={{position:"absolute",bottom:16,left:24,pointerEvents:"none",display:"flex",gap:16}}>{[["Drag","Rotate"],["Shift+Drag","Pan"],["Scroll","Zoom"]].map(function(pr){return(<div key={pr[1]} style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:10,color:TS,background:ST,padding:"3px 8px",borderRadius:4,fontWeight:600}}>{pr[0]}</span><span style={{fontSize:11,color:TS}}>{pr[1]}</span></div>);})}</div>
      </div>
      <div style={{width:340,background:PB,borderLeft:"1px solid "+BC,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:"20px 22px 16px",borderBottom:"1px solid "+BC}}><div style={{fontSize:10,fontWeight:700,letterSpacing:"0.18em",color:A,textTransform:"uppercase"}}>Configure</div></div>
        <div style={{flex:1,overflowY:"auto",padding:"16px 18px",overscrollBehavior:"contain"}}>
          <Sec title="Equipment Type"><div style={{display:"flex",flexDirection:"column",gap:6}}>{BOOTH_TYPES.map(function(t){return(<button key={t.value} onClick={function(){switchType(t.value);}} style={{padding:"12px 14px",border:"2px solid "+(config.boothType===t.value?A:BC),borderRadius:8,background:config.boothType===t.value?"rgba(249,115,22,0.1)":"transparent",color:config.boothType===t.value?A:TS,fontSize:13,fontWeight:600,cursor:"pointer",textAlign:"left",transition:"all 0.2s"}}>{t.label}</button>);})}</div></Sec>
        ...
        <div style={{padding:"16px 18px",borderTop:"1px solid "+BC,background:SB}}><button style={{width:"100%",padding:"12px 16px",background:A,color:"#fff",border:"none",borderRadius:8,fontSize:14,fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase",cursor:"pointer"}} onMouseEnter={function(e){e.target.style.background="#ea6a10";}} onMouseLeave={function(e){e.target.style.background=A;}}>Request Quote</button></div>
      </div>
